...
int main()
{
    ...
    assert(initials("mario Martinez Alex") == "MMA");
...
int countAlpha(string strA)// counts all the letters in a string
{
    int count = 0;
    for(int i = 0; i < strA.length(); i++)
    {
...
int countWords(string strA)// counts the number of words in a string.
{
...
bool isPalindrome(string strA)//returns true or false if the words is a palindrome
{
    string reverse;
    string lowerCase;
    string initials;
